...
#   - Download here (note your installed version of Chrome): https://sites.google.com/a/chromium.org/chromedriver/downloads
#   - Extract the .exe from the .zip folder and place in a place you would like to keep it. It does not matter.
#   - Copy the path to where the chromedriver is, and paste it in the spot below. Make sure to replace any \ with \\ for proper syntax.
...
# Finds and clicks on the 'High School' option to ensure those are the rankings that are scraped by the program
hsButton = driver.find_element_by_xpath("//*[@id=\"myutr-app-wrapper\"]/div[3]/div/div[5]/div[2]/div/div/div[1]/div[1]/div/span[4]")
hsButton.click()

# Instantiates a blank list of names to be added onto as the program scrapes universaltennis.com
names = []
...
with open('ranks.csv', 'w', newline='') as ranks:
    ...
    # Each page contains 100 players.
    #       Precondition: 1 < pages <= 25
    pages = 5

    # Cycles through a preset amount of pages, scraping the names of the players and adding them to the 'names' list.
    for x in range(1, pages + 1):
        # Finds and clicks on the dropdown element that selects the proper range of players
        #       i.e. Clicks on '201-300' from the dropdown so that the 201st-300th ranked players show up
    ...
    # Once all the pages are scraped and the names are found, the program will now go to tennisrecruiting.net to find the age/class of players.
    # First, we will open up the automated Chrome browser at the following link:
    driver.get("https://www.tennisrecruiting.net/player.asp")

    # Then the program will go through every name in 'names' and find their tennisrecruiting.net profile
    for index, name in enumerate(names):
        ...
        # Determines whether or not searching/querying for the player lands directly on their page immediately
        onPage = False
        contents = driver.find_elements_by_xpath("//*[@id=\"CenterColumn\"]/h1")
        if len(contents) > 0:
            if(contents[0].text == "PLAYER RECORD"):
                # If the webpage has this element (which is unique to player profiles), the program has determined that it is on the player's page
                onPage = True

        # If the program is not on a player page (i.e. there are multiple people with similar names), it will select the first result from the search results.
        #       The first result is generally the best matching name and/or highest ranked player, so this works most of the time (it has not failed yet, but I can see it happening).
        if not onPage:
            link = driver.find_element_by_xpath("//*[@id=\"CenterColumn\"]/div[2]/table/tbody/tr[2]/td[1]/b/a")
            link.click()
            time.sleep(1)
